...
def MultiplePositions(radius, velocity, time, dt):
    ...
    xouter = CalculatePosition(
        radius[len(radius) - 1], velocity[len(radius) - 1], time, dt
    )[0]
...
def MakeAnimation(
    radius, velocity, time, dt, filename, title, xlim=1, ylim=1, size=False, masses=None
):
    """
    ...
    images = []
    for i in range(1, len(storedtimes) + 1):
        image_path = "images/{}.png".format(i)
        images.append(imageio.imread(image_path))
    gif_path = f"{filename}"
    imageio.mimsave(gif_path, images, fps=25)

    # # Convert the images to a gif using ffmpeg
    # subprocess.call(["ffmpeg", "-i", "images/%d.png", "{filename}.gif"])
